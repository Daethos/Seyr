...
const Phaser = ({ ascean }: Props) => {
    const [phaserModalShow, setPhaserModalShow] = useState<boolean>(false);

    return (
        <>
        <Button variant="outline" 
            className="" 
            style={{ color: ascean?.hardcore ? 'red' : 'gold', marginLeft: -12 + 'px', fontWeight: 600 }}
            onClick={() => setPhaserModalShow(true)}
        ><svg xmlns="http://www.w3.org/2000/svg"  width="28" height="28" fill="currentColor" viewBox="0 0 1000 1000">
        ...
        <Modal 
            show={phaserModalShow}
            onHide={() => setPhaserModalShow(false)}
            centered
        ...
        <Modal.Body id="modal-delete" className="equipment-modal" style={{ color: "#fdf6d8" }}>
            ...
            <AsceanImageCard 
                ...
                key={ascean._id} 
            />
        <Nav.Link as={NavLink} to={'/Story/' + ascean._id} style={{ color: ascean.hardcore ? 'red' : 'blue', fontSize: "32px", fontWeight: 900, textShadow: "1px 1px 1px purple" }} className='btn btn-lg btn-outline-black mb-1 update-links'>
            Play as {ascean.name}
        </Nav.Link>
        </Modal.Body>
        </Modal>
