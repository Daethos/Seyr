...
export const AsceanModal = ({ ascean, link, symbol }: AsceanModalProps) => {
    ...
    return (
        <>
        <Button variant="outline" style={{ color: ascean?.hardcore ? 'red' : 'gold', marginLeft: -14 + 'px', fontWeight: 600 }} onClick={() => setShow(true)}>{symbol}</Button>
        <Modal show={show} onHide={() => setShow(false)} centered aria-labelledby="contained-modal-title-vcenter" id="modal-delete" style={{ fontFamily: "Cinzel" }}>
            <Modal.Header style={{ fontSize: "28px", color: "gold" }}> {ascean.name}
            ...
            <Modal.Body id="modal-delete" className="equipment-modal" style={{ color: "#fdf6d8" }}>
                <div className='creature-heading'><h2 style={{ fontSize: "18px", color: "gold" }}>{ascean.description}</h2></div>
