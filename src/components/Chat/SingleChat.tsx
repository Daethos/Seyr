...
const SingleChat = ({ fetchAgain, setFetchAgain, user, selectedChat, setSelectedChat, notification, setNotification, socketConnected, isTyping, socket  }: Props) => {
    ...
    const textSize = () => {
        ...
        if (width < 576) {
            if (newMessage.length > 105) {
                return '126px';
            } else if (newMessage.length > 70) {
                return '84px';
            } else {
                return '42px'; 
            };
        ...
        { selectedChat ? (
            <>
                ...
                <div className="chat-footer mt-3">
                    <Form.Control 
                        as="textarea" id='message-text' style={{ width: '80%', height: textSize() }}
                ...
                { loading 
                    ? <Loading Modal={true} />
                    : <Button variant="" style={{ color: 'red', fontSize: '18px', position: 'absolute', left: '82.5%' }} onClick={sendMessage}>Submit</Button>
                }
