...
const FeelingsCard = ({ loggedUser, ascean }: Props) => {
  ...
  const getAscean = async () => {
        try {
            const response = await asceanAPI.getOneAsceanLight(ascean._id);
  ...
  return (
    ...
    <OverlayTrigger placement="bottom" delay={{ show: 250, hide: 400 }} overlay={renderDoubleDislikesTooltip}>
      <svg onClick={doubleDislikeHandler} color={doubleDislikeColor} xmlns="http://www.w3.org/2000/svg" name="double-dislike" width="16" height="16" fill="currentColor" 
        className="bi bi-hand-thumbs-down feeling-icon mb-4" viewBox="0 0 16 16" id={likedIndex > -1 || dislikedIndex > -1 ? 'disable-feeling' : ''}>
      { doubleDislikedIndex > -1 
