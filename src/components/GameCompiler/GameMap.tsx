...
const GameMap = ({ mapData }: MapProps) => {
    ...
    //     const discoveredTiles = new Set<string>(); // initialize a set to store discovered tile coordinates
      
    //     // loop through the tiles and draw the discovered ones onto the canvas
    //     for (let x = playerX - 10; x <= playerX + 10; x++) { // only draw tiles within a certain range of the player's position
    //       for (let y = playerY - 10; y <= playerY + 10; y++) {
    //         const tile = mapData?.map?.[x]?.[y];
    //         if (tile && discoveredTiles.has(`${x},${y}`)) { // check if the tile exists and has been discovered
    //           const color = tile.color || 'gray';
    //           const offsetX = canvasWidth / 2 + (x - playerX) * tileSize; // adjust the position based on the player's position
    //           const offsetY = canvasHeight / 2 + (y - playerY) * tileSize;
    //           ctx.fillStyle = color;
    ...
    //     // function to mark a tile as discovered and redraw it
    //     function discoverTile(x: number, y: number) {
    //       discoveredTiles.add(`${x},${y}`);
    //       const tile = mapData?.map?.[x]?.[y];
    //       if (tile) {
    //         const color = tile.color || 'gray';
    //         const offsetX = canvasWidth / 2 + (x - playerX) * tileSize;
    //         const offsetY = canvasHeight / 2 + (y - playerY) * tileSize;
    //         ctx.fillStyle = color;
    //         ctx.fillRect(offsetX, offsetY, tileSize, tileSize);
    //       }
    ...
    //     // mark the player's current tile as discovered and redraw it
    //     discoverTile(playerX, playerY);
      
    //     // add event listeners to mark newly occupied tiles as discovered and redraw them
    //     canvas.addEventListener('click', (event) => {
    //       const clickX = event.offsetX / tileSize + playerX - canvasWidth / (2 * tileSize);
    //       const clickY = event.offsetY / tileSize + playerY - canvasHeight / (2 * tileSize);
    //       const roundedX = Math.round(clickX);
    //       const roundedY = Math.round(clickY);
    //       discoverTile(roundedX, roundedY);
    //     });
    // };
      
    
    // function drawMap(ctx: CanvasRenderingContext2D, mapData: MapData, canvas: HTMLCanvasElement): void {
    //     const tileSize = 2; // set the tile size to 16 pixels
    ...
    //     // loop through the tiles and draw them onto the canvas
    //     for (let x = -100; x <= 100; x++) {
    //         for (let y = -100; y <= 100; y++) {
    //             const tile = mapData?.map?.[x + 100]?.[y + 100];
    //             if (tile) {
    //                 console.log(tile, 'drawMap triggered');
    //                 const color = tile.color || 'gray'; // set the tile color to gray if no color is specified
    //                 const offsetX = canvasWidth / 2 + x * tileSize; // calculate the tile position on the canvas
    //                 const offsetY = canvasHeight / 2 + y * tileSize;
    //                 ctx.fillStyle = color;
